...
from .rules import Rule, Version, RuleSet, RuleHandler
...
class Includer(ABC):
    ...
    def __init__(self, fs: FileSystem = None, software_type: str = None):
        ...
        # Additional volume mount strings generated during assembly (e.g., for include-dir redirects)
        self._additional_volumes: List[str] = []
...
class InternalImage(Image, ABC):
    ...
    Rule Handlers:
        Subclasses can register custom rule handlers to process custom rule types.

        Example:
            class MyImage(InternalImage):
                @classmethod
                def _register_rule_handlers(cls):
                    cls.register_rule_handler("feature", cls._handle_feature_rule)

                @staticmethod
                def _handle_feature_rule(entry, result):
                    result["extras"].setdefault("features", []).append(entry.val)
    """

    # Class-level registry for custom rule handlers
    # Key: rule set type, Value: handler function
    _rule_handlers: Dict[str, "RuleHandler"] = {}

    @classmethod
    def register_rule_handler(cls, set_type: str, handler: "RuleHandler") -> None:
        """
        Register a custom rule handler for this image class.

        Args:
            set_type: The rule set type (e.g., "feature", "plugin")
            handler: Function with signature (entry: RuleEntry, result: dict) -> None
        """
        # Use class-specific registry to avoid affecting parent/sibling classes
        if not hasattr(cls, '_rule_handlers') or cls._rule_handlers is InternalImage._rule_handlers:
            cls._rule_handlers = dict(InternalImage._rule_handlers)
        cls._rule_handlers[set_type] = handler
        logger.debug(f"[{cls.__name__}] Registered rule handler for '{set_type}'")

    @classmethod
    def get_rule_handlers(cls) -> Dict[str, "RuleHandler"]:
        """
        Get all registered rule handlers for this class.

        Returns:
            Dict mapping rule set types to handler functions
        """
        return dict(cls._rule_handlers)
    ...
    def _generate_deps_from_rules(self):
        """
        Parse ruleset to determine base image and dependencies.

        Supports both new list format and legacy dict format.
        Format: [{"range": "...", "set": "valid|base|dep|custom", "val": "..."}]

        Custom rule types are handled by registered handlers (see register_rule_handler).
        """
        if not self.software or not self.version:
            logger.critical(f"[{self.name}] No Software or Version defined in Image")
            return
        ...
        if isinstance(rules_data, dict):
            ruleset = RuleSet.from_legacy_dict(rules_data)
        else:
            ruleset = RuleSet(rules_data)

        version_obj = Version(self.version)

        # Get custom handlers from class registry
        handlers = self.get_rule_handlers()
        result = ruleset.evaluate(version_obj, handlers=handlers)

        # Validate version
        if not result["valid"]:
            raise ImageDefinitionError(
                f"[{self.name}] Version '{self.version}' is not valid according to ruleset."
            )
        ...
        if result["base"]:
            ...
            if ":" in base_val:
                # Complete image tag (e.g., "3.12-slim", "python:3.12-slim")
                # Check if it's "image:tag" or just "tag with colon"
                parts = base_val.rsplit(":", 1)
                if len(parts) > 1:
                    # Looks like "image:tag" format
                    self.base_image = base_val
                    self.os = parts[0]
                    self.os_version = parts[1] if len(parts) > 1 else ""
                else:
                    # Just a tag with special chars, combine with self.os
                    self.os_version = base_val
                    self.base_image = f"{self.os}:{self.os_version}"
            else:
                # Just version number, combine with self.os (e.g., "22.04")
                self.os_version = base_val
                self.base_image = f"{self.os}:{self.os_version}"
            logger.debug(f"[{self.name}] Base image set to '{self.base_image}'")
        ...
        for dep in result["deps"]:
            ...
            logger.debug(f"[{self.name}] Added dependency '{dep}' from rule")

        # Process custom rule results (for subclasses to override)
        if result["extras"]:
            self._process_rule_extras(result["extras"])

    def _process_rule_extras(self, extras: Dict[str, Any]) -> None:
        """
        Process custom rule results from rule evaluation.

        Override this method in subclasses to handle custom rule types.

        Args:
            extras: Dict containing results from custom rule handlers
        """
        pass
    ...
    def _chsrc_install(self) -> str:
        ...
        install_cmd = (
            ...
            "        sed -i 's|archive.ubuntu.com|archive.ubuntu.com|g' /etc/apt/sources.list 2>/dev/null || true; \\\n"
...
class MasterBehavior(Behavior, ABC):
    ...
    Attributes:
        zone_obj: Zone object for domain name handling
        zone_file_key: Zone file key (e.g., "com" for "db.com") - used for grouping
        zone (inherited from Behavior): Record name (e.g., "www", "@")
    """
    ...
    def __init__(self, zone: str, args_str: str):
        ...
        self.zone_obj = Zone(zone)
        self.zone_file_key = self.zone_obj.label
    ...
    def _get_rname(self, origin: str) -> str:
        """
        Get FQDN from origin name using Zone.

        Args:
            origin: Record name (can be '@', relative name, or FQDN)

        Returns:
            FQDN string
        """
        if origin == '@':
            return self.zone_obj.fqdn
        elif origin.endswith('.'):
            return origin  # Already FQDN
        else:
            return (self.zone_obj / origin).fqdn
    ...
    def generate(
        ...
        rname = self._get_rname(self.zone)
        ...
        if self.record_type in ("A", "AAAA"):
            ...
            for target in self.targets:
                ...
                if target_ip:
                    ...
                    # External
                    records.append(
                        RR(rname=rname, rtype=rtype_id, rdata=NS(self._get_rname(target)), ttl=self.ttl)
                    )

        elif self.record_type == "CNAME":
            for target_domain in self.targets:
                records.append(
                    RR(rname=rname, rtype=rtype_id, rdata=CNAME(self._get_rname(target_domain)), ttl=self.ttl)
                )
            ...
            rdata_val = [self._get_rname(t).encode("utf-8") for t in self.targets]
            records.append(
                RR(
                    rname=rname,
                    rtype=rtype_id,
    ...
    def get_rname(origin: str, zone: str) -> str:
        """Get the full record name from origin and zone (deprecated, use _get_rname)"""
        if origin == '@':
            rname = zone
