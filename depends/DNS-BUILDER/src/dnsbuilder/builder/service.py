...
class ServiceHandler:
    ...
    def _assemble_configurations(self, fragments: List['ConfigFragment']):
        ...
        # Inject additional volume mounts generated by Includer (e.g., include-dir redirects)
        if includer._additional_volumes:
            for vol_str in includer._additional_volumes:
                self.processed_volumes.append(vol_str)
                logger.debug(
                    f"[{self.service_name}] Added additional volume mount: {vol_str}"
                )
