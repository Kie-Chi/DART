...
class BindMasterBehavior(MasterBehavior):
    ...
    def get_section_params(self, zone_name: str) -> dict:
        ...
        For BIND, the zone name in 'zone' statements must be in FQDN format.
        For root zone, use '.' instead of 'root'.

        Args:
            zone_name: Zone name (may be 'root' for root zone)

        Returns:
            Dict with 'name' key containing the FQDN zone name
        """
        # Convert 'root' to '.' for BIND zone statements
        if zone_name == "root":
            zone_name = "."
