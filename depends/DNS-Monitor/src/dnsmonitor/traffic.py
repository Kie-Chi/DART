...
class OptimizedTrafficMonitor:
    ...
    def __init__(self, config: TrafficConfig, mode: MonitorMode = MonitorMode.MIXED):
        ...
        # Adjust mode based on write_pcap config
        if not config.write_pcap and self.mode in [MonitorMode.PCAP_ONLY, MonitorMode.MIXED]:
            self.mode = MonitorMode.DISPLAY_ONLY
            self.logger.info("PCAP writing disabled via config, switching to DISPLAY_ONLY mode")
        ...
        self.pcap_write_count = 0  # Counter for periodic flush
        self.flush_interval = 100  # Flush every N packets
    ...
    def _init_pcap_writer(self) -> None:
        try:
            ...
            if self.pcap_handle:
                ...
                if self.pcap_dumper is None:
                    raise RuntimeError(f"dump_open() returned None for {self.current_pcap_file}")
                ...
                try:
                    ...
                    with self.stats_lock:
                        ...
                        if self.dropped_packets % 100 == 0:
                            self.logger.warning(f"PCAP write queue full, dropped {self.dropped_packets} packets")
    ...
    def _pcap_writer_worker(self) -> None:
        ...
        while self.running.is_set():
            try:
                ...
                if self.pcap_dumper:
                    ...
                    self.pcap_write_count += 1

                    # Periodic flush to ensure data is written to disk
                    if self.pcap_write_count % self.flush_interval == 0:
                        try:
                            if hasattr(self.pcap_dumper, 'flush'):
                                self.pcap_dumper.flush()
                        except Exception as e:
                            self.logger.debug(f"PCAP flush error: {e}")
        ...
        if self.pcap_dumper and hasattr(self.pcap_dumper, 'flush'):
            try:
                self.pcap_dumper.flush()
                self.logger.debug("Final PCAP flush completed")
            except Exception as e:
                self.logger.debug(f"Final PCAP flush error: {e}")
